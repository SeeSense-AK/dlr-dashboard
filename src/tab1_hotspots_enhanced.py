...
def render_summary_panel(hotspots_selected):
    """Render high-level summary panel with color-coded metrics"""
    
    total_hotspots = len(hotspots_selected)
    ...
    for _, row in hotspots_selected.iterrows():
        score = row.get('normalized_score', 0)
        if score >= 80:
            critical_count += 1
        elif score >= 60:
            medium_count += 1
...
def render_compact_hotspot_list(hotspots_selected):
    ...
    # List Items
    for i, row in hotspots_selected.iterrows():
        # Extract Data
        hotspot_name = row.get('hotspot_name', 'Hotspot')
        urgency_val = row.get('normalized_score', 0)
        urgency_score = f"{urgency_val:.1f}%"
        
        # Try finding location in various fields
        location = "Unknown"
        if 'identification.street_name' in row:
            location = row['identification.street_name']
        elif 'road_name' in row:
        ...
        # Event Type
        event_type = 'N/A'
        if 'sensor_data.event_type' in row:
            event_type = row['sensor_data.event_type']
        elif 'dominant_category' in row:
             event_type = row['dominant_category']

        # Riders / Devices
        device_count = "-"
        if 'sensor_data.device_count' in row:
             device_count = int(row['sensor_data.device_count'])
        
        # Reports
        user_reports = 0
        if 'collision_reports.total_count' in row:
             user_reports = int(row['collision_reports.total_count'])
        elif 'report_count' in row:
             user_reports = int(row['report_count'])
        ...
        with col8:
            if st.button("View", key=f"btn_{i}", width='stretch'):
...
def render_hotspot_details_page():
    ...
    # source = st.session_state.get('selected_source') # Removed source logic
    ...
    # Valid keys check
    location = row.get('identification.street_name') or row.get('road_name') or 'Unknown'
    if isinstance(location, list):
        location = " / ".join(location)
        
    lat = row.get('identification.latitude') or row.get('center_lat')
    lng = row.get('identification.longitude') or row.get('center_lng')
    
    score_val = row.get('normalized_score', 0)
    urgency_score = f"{score_val:.1f}%"
    
    event_type = row.get('sensor_data.event_type') or row.get('dominant_category') or 'N/A'
    reports = row.get('collision_reports.total_count') or row.get('report_count') or 0
    ...
    with col_map:
        try:
            ...
            # Try finding geometry in standard GeoJSON-derived fields or custom structure
            # Logic: IF geometry exists, draw it.
            
            # Check for corridor_data.geometry (Top 30 style) or geometry (Corridor style)
            geom_type = str(row.get('corridor_data.geometry.geometry_type', ''))
            coords = row.get('corridor_data.geometry.coordinates')
            
            # If not in corridor_data, check root 'geometry' field if it exists and looks like list of coords
            if not coords and 'geometry' in row and isinstance(row['geometry'], list):
                # Assume standard polygon coords list of [lat, lng] or [[lon, lat]]?
                # Based on previous code, corridor data geometry was list of [lon, lat]
                coords = row['geometry']
                geom_type = 'Polygon' # Force polygon for these legacy corridor fields
            ...
            if isinstance(coords, list):
                ...
                if geom_type == 'Polygon':
                    ...
                    # Handle difference in structure
                    if len(coords) > 0 and isinstance(coords[0], list) and isinstance(coords[0][0], list):
                        # GeoJSON Polygon [[[lon, lat], [lon, lat]...]]
                         poly_coords = [(p[1], p[0]) for p in coords[0]]
                    else:
                        # Simple Polygon [[lon, lat], [lon, lat]...]
                        poly_coords = [(p[1], p[0]) for p in coords]
    ...
    with st.spinner("Generating analysis..."):
        ...
        # hotspot_data['source'] = source # Source no longer relevant
        ...
        try:
            ...
            user_comments = (hotspot_data.get('sample_descriptions') or 
                           ...
                           row.get('narrative.sample_descriptions', []) or
                           extract_user_comments(hotspot_data))
    ...
    with col_pdf2:
        if st.button("📄 Save as PDF", width='stretch', type="primary"):
            try:
...
def load_preprocessed_data():
    ...
    try:
        with open(data_dir / "dlr-hotspots.json", 'r') as f:
    ...
    # except FileNotFoundError:
    #     st.warning("Abnormal events data not found.")
    abnormal_events_df = pd.DataFrame() # Return empty for now
    return top_30_df, abnormal_events_df

def select_top_hotspots(hotspots_df, total_count=10):
    """
    Select top hotspots based on score from unified dataframe.
    """
    ...
    # Standardize Score Column (Row-wise Coalesce)
    # 1. Try composite_score * 100 (Top 30 style)
    if 'scores.composite_score' in df.columns:
        df['s1'] = df['scores.composite_score'] * 100
    else:
        df['s1'] = None
        
    # 2. Try weighted_score (Corridor style)
    if 'weighted_score' in df.columns:
        df['s2'] = df['weighted_score']
    else:
        df['s2'] = None
        
    # 3. Try avg_severity as fallback (Raw Corridor style)
    if 'avg_severity' in df.columns:
        df['s3'] = pd.to_numeric(df['avg_severity'], errors='coerce')
    else:
        df['s3'] = None

    # Coalesce: Use s1 if valid, else s2, else s3, else 0
    df['normalized_score'] = df['s1'].fillna(df['s2']).fillna(df['s3']).fillna(0)
    
    # Clean up temp columns
    df = df.drop(columns=['s1', 's2', 's3'])
        
    # Sort by score descending
    df = df.sort_values('normalized_score', ascending=False)
    
    # Take Top N
    top_n = df.head(total_count).copy()
    
    # Assign names based on rank
    # This ensures Hotspot 1 is always the highest scored one in the list
    top_n['hotspot_name'] = [f"Hotspot {i+1}" for i in range(len(top_n))]
...
def create_popup_html(row):
    ...
    # Extract basic info
    location = row.get('identification.street_name') or row.get('road_name') or 'Unknown Location'
    if isinstance(location, list):
         location = " / ".join(location)
    ...
    lat = row.get('identification.latitude') or row.get('center_lat')
    lon = row.get('identification.longitude') or row.get('center_lng')
...
def create_hotspot_map(hotspots_selected, 
                       abnormal_events_df=None, show_heatmap=False):
    """Create Folium map with all hotspots and optional heatmap"""
    
    # Initialize map centered on DLR (Dún Laoghaire)
    m = folium.Map(
        location=[53.2913, -6.1360], # DLR County Hall / Town Centre
        zoom_start=13,
    ...
    # Add hotspots
    for idx, row in hotspots_selected.iterrows():
        color = get_color_by_score(row.get('normalized_score', 0))
        popup_html = create_popup_html(row)
        
        # Check for geometry presence
        has_geometry = False
        
        # 1. Try Standard GeoJSON flattened columns
        geom_type = str(row.get('geometry.type', ''))
        coords = row.get('geometry.coordinates')
        
        # 2. Try nested legacy path if not found
        if not coords:
            geom_type = str(row.get('corridor_data.geometry.geometry_type', ''))
            coords = row.get('corridor_data.geometry.coordinates')

        # 3. Try raw 'geometry' column (legacy corridor list of lists)
        # Note: only stick to this if it is a LIST, not NaN
        if not coords and 'geometry' in row and isinstance(row['geometry'], list):
             coords = row['geometry']
             geom_type = 'Polygon' # Fallback for old corridor style

        # If we have coordinates, assume we found geometry
        if isinstance(coords, list):
            has_geometry = True
            
            if geom_type == 'Polygon':
                # Handle variety of Polygon coordinate structures
                if len(coords) > 0 and isinstance(coords[0], list) and isinstance(coords[0][0], list):
                    # GeoJSON Polygon [[[lon, lat], [lon, lat]...]]
                     poly_coords = [(p[1], p[0]) for p in coords[0]]
                else:
                ...
                folium.Polygon(
                    ...
                    tooltip=f"{row.get('identification.street_name', row.get('hotspot_name'))}",
                    color=color,
                    fill=True,
                    fillColor=color,
                    fillOpacity=0.4,
...
def render_tab1_enhanced():
    ...
    # Load data
    try:
        hotspots_df, abnormal_events_df = load_preprocessed_data()
    except Exception as e:
    ...
    show_heatmap = False # Forced off
    
    # Select top hotspots
    hotspots_selected = select_top_hotspots(
        hotspots_df, total_hotspots
    )
    
    # 1. Summary Panel
    render_summary_panel(hotspots_selected)
    ...
    render_compact_hotspot_list(hotspots_selected)
